...
class DockerRemediator:
    ...
    def _classify(self, payload: dict[str, Any]) -> RemediationDecision | None:
        ...
        def match(pattern: str) -> bool:
            return pattern in rule_name or pattern in output
        ...
        if match("drop and execute new binary in container"):
            return RemediationDecision(
                ...
                action="stop",
                reason="New binary dropped and executed inside container",
            )

        if match("run shell untrusted"):
            return RemediationDecision(
                technique_id="T1059",
                technique_name="Command and Scripting Interpreter",
                action="stop",
                reason="Untrusted shell execution detected inside container",
            )

        if match("netcat remote code execution in container"):
            return RemediationDecision(
                technique_id="T1059",
                technique_name="Command and Scripting Interpreter",
                action="stop",
                reason="Netcat-based remote code execution detected inside container",
            )

        if match("ptrace attached to process"):
            return RemediationDecision(
                technique_id="T1068",
                technique_name="Exploitation for Privilege Escalation",
                action="stop",
                reason="ptrace attach detected — possible privilege escalation attempt",
            )

        if match("read sensitive file untrusted"):
            return RemediationDecision(
                technique_id="T1083",
                technique_name="File and Directory Discovery",
                action="alert",
                reason="Sensitive file read by untrusted process",
            )
